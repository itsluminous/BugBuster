...
function actions(dispatch) {
    return {
        ...
        systemInfoRetrievalSuccess: systemInfo => dispatch({
            type: 'SYSTEM_INFO_RETRIEVAL_SUCCESS',
            payload: { systemInfo },
        }),
        systemInfoRetrievalFailure: error => dispatch({
            type: 'SYSTEM_INFO_RETRIEVAL_FAILURE',
            payload: { error },
        }),
        userInfoRetrievalSuccess: userInfoResponse => dispatch({
        ...
        reportsRetrievalSuccess: reports => dispatch({
            type: 'REPORTS_RETRIEVAL_SUCCESS',
            payload: { reports },
        }),
        reportsRetrievalFailure: error => dispatch({
            type: 'REPORTS_RETRIEVAL_FAILURE',
            payload: { error },
        }),
        screenshotCaptureSuccess: screenshot => dispatch({
            type: 'SCREENSHOT_CAPTURE_SUCCESS',
            payload: { screenshot },
        }),
        screenshotCaptureFailure: error => dispatch({
        ...
        reportProcessingFailure: (processingId, error) => dispatch({
            type: 'REPORT_PROCESSING_FAILURE',
            payload: { processingId, error },
        }),
        onNetworkError: error => dispatch({
            type: 'NETWORK_GET_RESPONSE_BODY_FAILURE',
            payload: { error },
        }),
        onRecordingFailure: error => dispatch({
            type: 'RECORDING_FAILURE',
            payload: { error },
        }),
        recordingStartSuccess: (tab, stop) => dispatch({
            type: 'RECORDING_START_SUCCESS',
            payload: { tab, stop },
        }),
        recordingCancel: () => dispatch({
            type: 'RECORDING_CANCEL',
        }),
        recordingStartFailure: error => dispatch({ type: 'RECORDING_START_FAILURE', payload: { error } }),
        recordingStop: (video, tab) => dispatch({
            type: 'RECORDING_STOP',
            payload: { video, tab },
        }),
        integrationsRetrievalFailure: error => dispatch({
            type: 'INTEGRATIONS_RETRIEVAL_FAILURE',
            payload: { error },
        }),
        jiraIntegrationRetrievalSuccess: jira => dispatch({
            type: 'JIRA_INTEGRATION_RETRIEVAL_SUCCESS',
            payload: { jira },
        }),
        jiraIntegrationRetrievalFailure: error => dispatch({
            type: 'JIRA_INTEGRATION_RETRIEVAL_FAILURE',
            payload: { error },
        }),
        trelloIntegrationRetrievalSuccess: trello => dispatch({
            type: 'TRELLO_INTEGRATION_RETRIEVAL_SUCCESS',
            payload: { trello },
        }),
        trelloIntegrationRetrievalFailure: error => dispatch({
...
},{"lodash":25,"urijs":32}],3:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const csrf = csrf_1.initCsrf(window);
const Api = api_1.api(window, settings_1.settings, csrf);
...
subscribe_1.subscribe(settings_1.settings, store, navigator, screen, browser.tabs, browser.tabCapture, chrome.desktopCapture, chrome.browserAction, MediaRecorder, Blob, chrome, Api, notifications_1.notifications(chrome.notifications, chrome.tabs), browser_detection_1.capabilities);
chrome.tabs.onRemoved.addListener(tabId => store.dispatch({ type: 'TAB_CLOSED', payload: { tabId } }));

},{"../browser-detection":23,"./api":2,"./csrf":4,"./notifications":7,"./settings":17,"./store":18,"./subscribe":19}],4:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
// Append an iframe to the iframe holder and dispatch when that iframe has posted a csrf token
function initCsrf(window) {
    let csrfToken;
    let resolve;
    let refreshing;
    const iframeSrc = `${window.baseUrl}/auth/extension?origin=${encodeURIComponent(window.location.origin)}`;
    window.addEventListener('message', (message) => {
        if (message.data.name !== 'csrfAuth')
            return;
        csrfToken = message.data.data;
        resolve();
    });
    function refresh() {
        if (refreshing)
            return refreshing;
        refreshing = new Promise((_resolve, reject) => {
            resolve = _resolve;
            const iframeHolder = window.document.getElementById('iframeHolder');
            Array.prototype.forEach.call(iframeHolder.querySelectorAll('iframe'), iframe => {
                iframeHolder.removeChild(iframe);
            });
    ...
    // Wait for the window to load, before which the iframeHolder doesn't exist
...
function notifications(chromeNotifications, chromeTabs) {
    chromeNotifications.onClicked.addListener(url => {
        chromeNotifications.clear(url);
...
function standardizeLogEvent(eventName, e) {
    if (eventName === 'entryAdded') {
        e.entry.timestamp = e.entry.timestamp / 1000;
        return e.entry;
    }
    if (eventName === 'consoleAPICalled') {
        e.timestamp = e.timestamp / 1000;
        // rename stackTrace to stack to satisfy older model
        if (e.stackTrace && !e.stack) {
            e.stack = e.stackTrace;
            delete e.stackTrace;
        }
        if (e.args && !e.parameters) {
            e.parameters = e.args;
            delete e.args;
        }
        return e;
    }
    if (eventName === 'exceptionThrown') {
        const ret = e;
        ret.level = 'error';
        const evt = e;
        let ts = evt.timestamp; // tslint:disable-line:no-let
        if (ts / 1000 > 1) {
            ts = evt.timestamp / 1000;
        }
        const details = evt.exceptionDetails;
        ret.stack = details.stackTrace;
        const exc = details.exception;
        if (exc && exc.description) {
            // if there arent newlines, use e.text, failing that blank
            const text = (exc.description.split('\n') || [e.text || ''])[0];
            ret.text = text;
        }
        e.timestamp = ts;
        return e;
...
function scriptIdsForMsg(msg) {
    if (!msg.stack)
        return;
    if (!msg.stack.callFrames)
        return;
    if (!msg.stack.callFrames.length)
        return;
    const ret = {};
    for (let i = 0, l = msg.stack.callFrames.length; i < l; i++) {
        const frame = msg.stack.callFrames[i];
        ret[frame.scriptId] = true;
    }
...
function startConsoleRecorder() {
    const buffer = []; // tslint:disable-line:readonly-array
    const scriptIds = {};
    const scriptMap = {};
    function checkUnique(message) {
        return !buffer.some(entry => JSON.stringify(entry) === message);
    }
    function indexScriptIDs(msg) {
        const scripts = scriptIdsForMsg(msg);
        if (!scripts)
            return;
        for (let i = 0, l = scripts.length; i < l; i++) {
            scriptIds[scripts[i]] = true;
        }
    }
    function swapScriptIDForURLHash() {
        for (let i = 0, l = buffer.length; i < l; i++) {
            ...
            for (let ii = 0, ll = msg.stack.callFrames.length; ii < ll; ii++) {
                const frame = msg.stack.callFrames[ii];
                const script = scriptMap[frame.scriptId];
                if (script) {
                    frame.scriptHash = script.hash;
                }
            }
        }
    }
    function calcSourceMaps() {
        const sourceMaps = []; // tslint:disable-line:readonly-array
        const scriptIdsArr = Object.getOwnPropertyNames(scriptIds);
        for (let i = 0, l = scriptIdsArr.length; i < l; i++) {
            const scriptId = scriptIdsArr[i];
            const m = scriptMap[scriptId];
            if (!m)
                continue;
            const sourceMapURL = m.sourceMapURL;
            if (!sourceMapURL)
                continue;
            sourceMaps.push(m);
        }
    ...
    function sourceMapHash() {
        const sourceMaps = calcSourceMaps();
        if (!sourceMaps.length)
            return null;
        const ret = {};
        for (let i = 0, l = sourceMaps.length; i < l; i++) {
            ret[sourceMaps[i].hash] = sourceMaps[i];
        }
        return ret;
    }
    function onEvent(type, data) {
        if (type === 'scriptParsed') {
            scriptMap[data.scriptId] = data;
            return;
        }
        if (!toLog.has(type))
            return;
        const msg = standardizeLogEvent(type, data);
        const encoded = JSON.stringify(msg);
        if (!checkUnique(encoded))
            return;
        indexScriptIDs(msg);
        buffer.push(msg);
    }
    function stop() {
        swapScriptIDForURLHash();
        const data = buffer.length ? JSON.stringify({ console_logs: buffer }) : null;
        return {
            data,
...
function startRecording(tab, type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, chrome, capabilities, dispatchBackgroundActions) {
    let videoReady = false; // tslint:disable-line:no-let
    let tabRecorderReady = false; // tslint:disable-line:no-let
    let videoRecorder;
    let tabRecorder;
    const onBothReady = lodash_1.once(() => dispatchBackgroundActions.recordingStartSuccess(tab, stop));
    const onError = lodash_1.once(error => {
        if (tabRecorder)
            tabRecorder.stop();
        if (videoRecorder)
            videoRecorder.stop();
        return dispatchBackgroundActions.recordingStartFailure(error);
    });
    try {
        videoRecorder = video_1.startVideoRecorder(tab, type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, capabilities, onError, error => {
            if (error)
            ...
            if (videoRecorder)
                videoRecorder.stop();
            return dispatchBackgroundActions.recordingCancel();
        });
        tabRecorder = tab_1.startTabRecorder(tab.id, chrome, capabilities, dispatchBackgroundActions.debuggerDetached, dispatchBackgroundActions.onNetworkError, error => {
            if (error)
                return onError(error);
    ...
    function stop() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const videoResult = videoRecorder.stop();
                const tabResult = tabRecorder.stop();
                dispatchBackgroundActions.recordingStop(yield videoResult, tabResult);
            }
            catch (error) {
                dispatchBackgroundActions.onRecordingFailure(error);
            }
        });
    }
}
...
},{"./tab":11,"./video":13,"lodash":25}],10:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
function redactURI(uri) {
    // https://en.wikipedia.org/wiki/Data_URI_scheme
    if (uri.substr(0, 5) !== 'data:') {
        // no redaction neccesary.
        return uri;
    }
    ...
    const header = uri.substr(0, commaPos + 1);
    return header + '<encoded data, redacted>';
}
const toLog = new Set([
    'loadingFinished',
    'loadingFailed',
    ...
    function onEvent(type, data) {
        ...
        switch (type) {
            ...
            case 'loadingFinished': {
                ...
                chrome.debugger.sendCommand({ tabId }, 'Network.getResponseBody', { requestId }, data => {
...
exports.startNetworkRecorder = startNetworkRecorder;
...
function attachDebugger(tabId, chrome, attachVersion, cb) {
    chrome.debugger.attach({ tabId }, attachVersion, () => {
        cb(chrome.runtime.lastError);
    });
}
function enableDomain(tabId, chrome, domain, cb) {
    chrome.debugger.sendCommand({ tabId }, domain + '.enable', () => {
        cb(chrome.runtime.lastError);
    });
}
function enableDomains(tabId, chrome, domains, cb) {
    const remainingDomains = new Set(domains);
    domains.forEach(domain => {
        enableDomain(tabId, chrome, domain, err => {
            if (err)
                return cb(err);
            remainingDomains.delete(domain);
            if (!remainingDomains.size)
                return cb();
        });
    });
}
function startTabRecorder(tabId, chrome, capabilities, onDebuggerDetached, onNetworkError, onInitialized) {
    if (capabilities.isFirefox) {
        throw new Error('TODO support firefox');
    }
    let debuggerDetached = false;
    const networkRecorder = network_1.startNetworkRecorder(tabId, chrome, onNetworkError);
    const consoleRecorder = console_1.startConsoleRecorder();
    function eventListener(tab, eventName, eventData) {
        if (tab.tabId !== tabId)
            return;
        const [domain, eventId] = eventName.split('.');
        switch (domain) {
            case 'Debugger':
            case 'Console':
            case 'Runtime':
            case 'Log':
                return consoleRecorder.onEvent(eventId, eventData);
            case 'Network':
                return networkRecorder.onEvent(eventId, eventData);
        }
    }
    // onDebuggerDetached triggers the recording to stop
    ...
    function stop() {
        ...
        return {
            ...
            console: consoleRecorder.stop(),
        };
    }
    const debuggerVersion = capabilities.chromeDebuggerAttach12 ? '1.2' : '1.1';
    const consoleDomains = capabilities.chromeLogRunTimeDomains
        ? ['Runtime', 'Log']
        : ['Console'];
    attachDebugger(tabId, chrome, debuggerVersion, err => {
        if (err)
            return onInitialized(err);
        enableDomains(tabId, chrome, ['Debugger', 'Network'].concat(consoleDomains), onInitialized);
    });
    return {
        stop,
    };
}
exports.startTabRecorder = startTabRecorder;
...
function asMinutes(seconds) {
    ...
    const secsString = secs >= 10 ? String(secs) : '0' + String(secs);
    return `${minutes}:${secsString}`;
}
function startTimer(browserAction, tab) {
    const tabId = tab.id;
    const startedAt = Date.now();
    let badgeText = ''; // tslint:disable-line:no-let
    // Wrap around browserAction.setBadgeText to only call it when the text has actually changed
    ...
    browserAction.setBadgeBackgroundColor({ color: [0, 0, 0, 255], tabId });
    setBadgeText('0:00');
    const interval = setInterval(() => {
        const now = Date.now();
        const elapsed = now - startedAt;
        setBadgeText(asMinutes(Math.floor(elapsed / 1000)));
    }, 20);
    return {
        stop() {
            setBadgeText('');
...
function startChromeVideoRecorder(tab, type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, capabilities, onRecordingFailure, onInitialized, onCancel) {
    let stream; // tslint:disable-line:no-let
    let videoRecorder; // tslint:disable-line:no-let
    let timer; // tslint:disable-line:no-let
    ...
    function stop() {
        ...
        return stopped.then(() => {
            return { frameTimes, blob: new Blob(frames) };
        });
    }
    function withStream(s) {
        try {
            ...
            videoRecorder.onstart = () => {
                timer = timer_1.startTimer(browserAction, tab);
            };
            ...
            Object.assign(error, {
                errorMessageForUser: 'The recording failed to start.',
            });
            stop();
            onInitialized(error);
        }
    ...
    if (type === 'tab') {
        if (capabilities.chromeDisableTabCapture) {
            throw new Error('A user should not be able to start a tab recording because tab capture is disabled');
        }
        tabCapture.capture(userMediaConstraints(tab), withStream);
    }
    else {
        desktopCapture.chooseDesktopMedia(['window', 'screen'], streamId => {
            if (!streamId)
                return onCancel();
...
exports.startChromeVideoRecorder = startChromeVideoRecorder;
function startVideoRecorder(tab, type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, capabilities, onRecordingFailure, onInitialized, onCancel) {
    if (capabilities.isFirefox) {
        throw new Error('TODO support firefox');
    }
    return startChromeVideoRecorder(tab, type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, capabilities, onRecordingFailure, onInitialized, onCancel);
}
exports.startVideoRecorder = startVideoRecorder;

},{"./timer":12}],14:[function(require,module,exports){
"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
...
},{"lodash":25}],15:[function(require,module,exports){
"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
...
function takeScreenshot(tab, tabs, dispatchBackgroundActions) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const screenshotUri = yield tabs.captureVisibleTab({ format: 'png' });
...
function logErrorAndReturnCopyForUser(settings, api, initialState, action) {
    ...
    if (!settings.isTestEnvironment) {
        console.error(error);
        const systemInfo = initialState.popup.connected
            ? initialState.popup.systemInfo
            : undefined;
    ...
    let message = typeof error === 'string'
        ? error
        : error.errorMessageForUser || error.message;
    if (!message)
    ...
    switch (action.type) {
        ...
        case 'POPUP_DISCONNECT': {
            return Object.assign(Object.assign({}, initialState), { popup: { connected: false }, userInfoResponse: initialState.userInfoResponse &&
                    initialState.userInfoResponse.loggedIn
                    ? initialState.userInfoResponse
                    : null, alert: null });
        }
        case 'CLICK_START_RECORDING_TAB':
        case 'CLICK_START_RECORDING_SCREEN': {
            if (initialState.recording.startedByUser)
                return initialState;
        ...
        case 'DEBUGGER_DETACHED': {
            // DEBUGGER_DETACHED is a weird one - it's detected by the background, but we treat it like the user stopped the recording
            if (!initialState.recording.started) {
                throw new Error("Recording hasn't started yet!");
            }
            return Object.assign(Object.assign({}, initialState), { recording: Object.assign(Object.assign({}, initialState.recording), { stoppedByUser: true }) });
        }
        case 'CLICK_TAKE_SCREENSHOT': {
            return Object.assign(Object.assign({}, initialState), { screenshot: {
                    requestedByUser: true,
                    screenshots: initialState.screenshot.screenshots,
                } });
        }
        case 'CLICK_DELETE_SCREENSHOT': {
            const screenshots = initialState.screenshot.screenshots.filter(screenshot => screenshot !== action.payload.screenshot);
            return Object.assign(Object.assign({}, initialState), { screenshot: { requestedByUser: false, screenshots }, report: !screenshots.length ? { started: false } : initialState.report });
        }
        case 'CLICK_DONE_TAKING_SCREENSHOTS': {
            return Object.assign(Object.assign({}, initialState), { report: reportStarted(initialState) });
        }
        case 'UPDATE_REPORT': {
            return Object.assign(Object.assign({}, initialState), { report: Object.assign(Object.assign({}, initialState.report), { details: Object.assign(Object.assign({}, initialState.report.details), action.payload.updates) }) });
        }
        case 'TAB_CLOSED': {
            const { recording } = initialState;
            const tabIsBeingRecorded = recording.started && recording.tab.id === action.payload.tabId;
            if (!tabIsBeingRecorded) {
                return initialState;
            }
            return Object.assign(Object.assign({}, initialState), { recording: Object.assign(Object.assign({}, recording), { stoppedByUser: true }) });
        }
        case 'CANCEL_REPORT': {
            return Object.assign(Object.assign({}, initialState), { recording: emptyState.recording, report: emptyState.report });
        }
        case 'CLICK_SUBMIT_REPORT': {
            const reportDetails = initialState.report.details;
            if (!reportDetails)
                throw new Error('Cannot submit a report without details');
            return Object.assign(Object.assign({}, initialState), { screenshot: emptyState.screenshot, recording: emptyState.recording, report: emptyState.report, reports: {
                    fetchedFromServer: initialState.reports.fetchedFromServer,
                    processed: (initialState.reports.processed || []).slice(0, settings.maxReportsToShow -
                        1 -
                        initialState.reports.processing.length),
                    processing: [
                        {
                            processingId: utils_1.randId(),
                            tab: initialState.recording.tab ||
                                initialState.screenshot.screenshots[0].tab,
                            systemInfo: initialState.popup
                                .systemInfo,
                            recorders: initialState.recording.recorders || null,
                            screenshots: initialState.screenshot.screenshots,
                            uploadingAssets: initialState.recording.uploadingAssets || null,
                            details: reportDetails,
                        },
                    ].concat(initialState.reports.processing),
                    failed: initialState.reports.failed,
                } });
        }
        case 'RETRY_SUBMIT_REPORT': {
            const [retry, failed] = lodash_1.partition(initialState.reports.failed, report => report.processingId === action.payload.processingId);
            return Object.assign(Object.assign({}, initialState), { reports: Object.assign(Object.assign({}, initialState.reports), { failed, processing: retry.concat(initialState.reports.processing) }) });
        }
        case 'CANCEL_RETRY_SUBMIT_REPORT': {
            const [, failed] = lodash_1.partition(initialState.reports.failed, report => report.processingId === action.payload.processingId);
            return Object.assign(Object.assign({}, initialState), { reports: Object.assign(Object.assign({}, initialState.reports), { failed }) });
        }
        case 'DISMISS_ALERT': {
            return Object.assign(Object.assign({}, initialState), { alert: null });
        }
        // Background Actions
        case 'ACTIVE_TAB_DETECTED': {
            if (!initialState.popup.connected)
                return initialState;
            return Object.assign(Object.assign({}, initialState), { popup: {
                    connected: true,
        ...
        case 'NO_ACTIVE_TAB_DETECTED': {
            return Object.assign(Object.assign({}, initialState), { alert: 'Could not connect with the active tab. Please retry.' });
        }
        case 'SYSTEM_INFO_RETRIEVAL_SUCCESS': {
            const { popup } = initialState;
            if (!popup.connected)
                return initialState;
            return Object.assign(Object.assign({}, initialState), { popup: Object.assign(Object.assign({}, popup), { systemInfo: action.payload.systemInfo }) });
        }
        case 'SYSTEM_INFO_RETRIEVAL_FAILURE': {
        ...
        case 'REPORTS_RETRIEVAL_SUCCESS': {
            const { processing, failed } = initialState.reports;
            const processingReportTitles = new Set(processing.map(report => report.details.title));
            const processed = action.payload.reports
                .filter(report => !processingReportTitles.has(report.title))
                .slice(0, settings.maxReportsToShow - processing.length);
            return Object.assign(Object.assign({}, initialState), { reports: { processing, processed, failed, fetchedFromServer: true } });
        }
        case 'REPORTS_RETRIEVAL_FAILURE': {
            if (action.payload.error.error_name === 'ErrUserNotLoggedIn') {
                if (initialState.userInfoResponse &&
                    !initialState.userInfoResponse.loggedIn) {
                    return initialState;
                }
                return Object.assign(Object.assign({}, initialState), { userInfoResponse: { loggedIn: false } });
            }
            return Object.assign(Object.assign({}, initialState), { alert: logErrorAndReturnCopyForUser(settings, api, initialState, action) });
        }
        case 'SCREENSHOT_CAPTURE_SUCCESS': {
            return Object.assign(Object.assign({}, initialState), { screenshot: {
                    requestedByUser: false,
                    screenshots: initialState.screenshot.screenshots.concat([
        ...
        case 'SCREENSHOT_CAPTURE_FAILURE': {
            return Object.assign(Object.assign({}, initialState), { screenshot: {
                    requestedByUser: false,
                    screenshots: initialState.screenshot.screenshots,
                }, alert: logErrorAndReturnCopyForUser(settings, api, initialState, action) });
        }
        case 'UPLOADING_ASSETS': {
            return Object.assign(Object.assign({}, initialState), { recording: Object.assign(Object.assign({}, initialState.recording), { uploadingAssets: action.payload.assets }) });
        }
        case 'REPORT_PROCESSING_SUCCESS': {
            let alert = initialState.alert;
            if (action.payload.networkTrafficUploadError) {
                // We want to log the error, but don't care about the copy it returns
                logErrorAndReturnCopyForUser(settings, api, initialState, {
                    type: action.type,
                    payload: { error: action.payload.networkTrafficUploadError },
                });
            ...
            return Object.assign(Object.assign({}, initialState), { reports: {
                    ...
                    processed: [action.payload.processedReport].concat(initialState.reports.processed || []),
                    failed: initialState.reports.failed,
                }, alert });
        }
        case 'REPORT_PROCESSING_FAILURE': {
            const [[failed], processing] = lodash_1.partition(initialState.reports.processing, report => report.processingId === action.payload.processingId);
            return Object.assign(Object.assign({}, initialState), { reports: Object.assign(Object.assign({}, initialState.reports), { processing, failed: [lodash_1.omit(failed, 'uploadingAssets')].concat(initialState.reports.failed) }) });
        }
        case 'RECORDING_START_SUCCESS': {
            return Object.assign(Object.assign({}, initialState), { recording: Object.assign(Object.assign({}, initialState.recording), { stop: action.payload.stop, tab: action.payload.tab, started: true, stopped: false }) });
        }
        case 'RECORDING_CANCEL': {
            return Object.assign(Object.assign({}, initialState), { recording: emptyState.recording });
        }
        case 'RECORDING_START_FAILURE': {
            return Object.assign(Object.assign({}, initialState), { recording: emptyState.recording, alert: logErrorAndReturnCopyForUser(settings, api, initialState, action) });
        }
        case 'RECORDING_STOP': {
            return Object.assign(Object.assign({}, initialState), { report: reportStarted(initialState), recording: {
                    ...
                    started: true,
                    stopped: true,
                    type: initialState.recording.type,
                    tab: initialState.recording.tab,
                    recorders: action.payload,
                } });
        }
        case 'NETWORK_GET_RESPONSE_BODY_FAILURE': {
            return Object.assign(Object.assign({}, initialState), { alert: logErrorAndReturnCopyForUser(settings, api, initialState, action) });
        }
        case 'JIRA_INTEGRATION_RETRIEVAL_SUCCESS': {
            return Object.assign(Object.assign({}, initialState), { jira: action.payload.jira });
        }
        case 'TRELLO_INTEGRATION_RETRIEVAL_SUCCESS': {
            return Object.assign(Object.assign({}, initialState), { trello: action.payload.trello });
        }
        case 'JIRA_INTEGRATION_RETRIEVAL_FAILURE':
        case 'TRELLO_INTEGRATION_RETRIEVAL_FAILURE':
        ...
        default: {
            ...
            if (!/^@@redux/.test(faultyAction.type)) {
                return Object.assign(Object.assign({}, initialState), { alert: `Unknown action type ${faultyAction.type}` });
            }
            return initialState;
        }
    }
}
function createStore(settings, api) {
    const boundReducer = reducer.bind(null, settings, api);
    const useReducer = settings.debug
        ? (initialState, action) => {
            const nextState = boundReducer(initialState, action);
            console.log('Store Update', { initialState, action, nextState });
            return nextState;
        }
        : boundReducer;
    return redux.createStore(useReducer);
}
exports.createStore = createStore;

},{"../utils":24,"./errors":5,"lodash":25,"redux":27}],19:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
function subscribe(settings, store, navigator, screen, tabs, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, chrome, api, notifications, capabilities) {
    ...
    return store.subscribe(() => {
        ...
        if (nextState.recording.startedByUser &&
            ...
            recording_1.startRecording(nextState.popup.activeTab, nextState.recording.type, navigator, screen, tabCapture, desktopCapture, browserAction, MediaRecorder, Blob, chrome, capabilities, dispatchBackgroundActions);
        }
        ...
        if (prevState.reports.processing.length &&
            prevState.reports.processing.length -
                nextState.reports.processing.length ===
                1 &&
            prevState.reports.failed.length === nextState.reports.failed.length &&
            nextState.reports.processed &&
            nextState.reports.processed.length &&
            nextState.reports.processed.length -
                (prevState.reports.processed
                    ? prevState.reports.processed.length
                    : 0) ===
                1 &&
            prevState.reports.processing[0].details.title ===
                nextState.reports.processed[0].title) {
...
},{"./actions":1,"./integrations":6,"./recording":9,"./report":14,"./reports":15,"./screenshot":16,"./system-info":20,"./tab":21,"./user":22}],20:[function(require,module,exports){
"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
...
function getMem(chrome) {
    if (!chrome.system)
        return Promise.resolve(undefined);
    return new Promise(resolve => chrome.system.memory.getInfo(resolve));
}
function getStorage(chrome) {
    if (!chrome.system)
        return Promise.resolve(undefined);
    return new Promise(resolve => chrome.system.storage.getInfo(resolve));
}
function getCPU(chrome) {
    if (!chrome.system)
        return Promise.resolve(undefined);
...
function getSystemInfo(navigator, chrome, tabs, tab) {
    return __awaiter(this, void 0, void 0, function* () {
        const d = new Date();
        const gettingMem = getMem(chrome);
...
function checkSystemInfo(navigator, chrome, tabs, tab, dispatch) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const systemInfo = yield getSystemInfo(navigator, chrome, tabs, tab);
            dispatch.systemInfoRetrievalSuccess(systemInfo);
        }
        catch (error) {
            Object.assign(error, {
                errorMessageForUser: 'Could not get system information.',
            });
            dispatch.systemInfoRetrievalFailure(error);
        }
    });
}
...
Object.defineProperty(exports, "__esModule", { value: true });
function checkActiveTab(tabs, dispatchBackgroundActions) {
    return __awaiter(this, void 0, void 0, function* () {
        const activeTabs = yield tabs.query({
            active: true,
            currentWindow: true,
        });
...
Object.defineProperty(exports, "__esModule", { value: true });
function checkUserInfo(api, dispatchBackgroundActions) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const userInfoResponse = yield api.getUserInfo();
            dispatchBackgroundActions.userInfoRetrievalSuccess(userInfoResponse);
        }
        catch (error) {
            dispatchBackgroundActions.userInfoRetrievalFailure(error);
        }
    });
}
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
function chromeMajorVersion(navigator) {
    const str = navigator.userAgent;
    const matches = str.match(/Chrome\/([\d\.]+)/);
    if (!matches || !matches[1])
        return null;
    const [version] = matches[1].split('.');
    if (!version)
        return null;
    return parseInt(version, 10) || null;
}
function detectCapabilities(browser, navigator) {
    const chromeVersion = chromeMajorVersion(navigator);
    const isFirefox = chromeVersion == null;
    const isChrome = !isFirefox;
    const chromeLogRunTimeDomains = isChrome && chromeVersion > 53;
    const chromeDebuggerAttach12 = isChrome && chromeVersion > 53;
    const chromeDisableTabCapture = isChrome && chromeVersion === 57;
    const chromeVersionTooOld = isChrome && chromeVersion < 47;
    const chromePopupTransitionBroken = isChrome && chromeVersion === 66;
    const consoleLogs = isChrome;
    return {
        isFirefox,
        isChrome,
        chromeLogRunTimeDomains,
        chromeDebuggerAttach12,
        chromeDisableTabCapture,
        chromeVersionTooOld,
        chromePopupTransitionBroken,
        consoleLogs,
    };
}
...
},{"_process":26,"symbol-observable":28}],28:[function(require,module,exports){
(function (global){
'use strict';
...
},{"./IPv6":30,"./SecondLevelDomains":31,"./punycode":33}],33:[function(require,module,exports){
(function (global){
